...
int ClientCreate(int port, int anyip)		 //create the client socket
{
...
int main()
{
	...
	while(exit == 0)
	{
		...
		int error = ClientCreate(port, 0); //calling the client create function for the provided credentials
		if(error == 1)
		{
			close(c_socket);
			printf("Connection Issue.\n");
